...
def main():
    ...
    # 读取出来的图像数据
    json_util = LoadData('stretch')
    json_data = json_util.load_data()
    print(json_data)
    data_arr = json_data['stretch']
    print(data_arr)
    print(data_arr[0])
    # print(data_arr[1])
    ...
    while True:
        ...
        if results.pose_landmarks is not None:
            ...
            # 识别成功，向前推进一帧
            tip_Inc_text, tip_Dec_text = compareAngle(data_arr[index], angle_json, 5)
            if len(tip_Inc_text) == 0 and len(tip_Dec_text) == 0:
                print("")
                print("=============", index, "================")
                print("识别的角度：", angle_json)
                print("标准的角度：", data_arr[index])
                index += 1

                print("")
                print("下一个要识别的角度: ", data_arr[index])

            for s in tip_Inc_text:
                result_str_l += s + "\n"
            for s in tip_Dec_text:
                result_str_r += s + "\n"

            if index >= len(data_arr):
                print("结束: ", index)
                break
            # 位置参数说明：
            # 图片
        ...
        # cv.waitKey(1000 / int(display_fps))  # 延迟
...
# 比较两个姿势角度数据是否相等
def compareAngle(standard_angle, angle_json, mistake=5):
    # 判断是否识别
    flag = True
    # 提示文字
    tipIncText = []
    ...
    if standard != -1 and angle not in interval(standard, mistake):
        if angle < standard - mistake:
            tipIncText.append("请增大左手臂与左侧腰之间的幅度")
        if angle > standard + mistake:
            tipDecText.append("请减小左手臂与左侧腰之间的幅度")

    standard = standard_angle['armpit']['right_armpit_angle']
    angle = angle_json['armpit']['right_armpit_angle']
    if standard != -1 and angle not in interval(standard, mistake):
        if angle < standard - mistake:
            tipIncText.append("请增大右手臂与右侧腰之间的幅度！")
        if angle > standard + mistake:
            tipDecText.append("请减小右手臂与右侧腰之间的幅度！")
    ...
    if standard != -1 and angle not in interval(standard, mistake):
        if angle < standard - mistake:
            tipIncText.append("请增大左手臂的幅度！")
        if angle > standard + mistake:
            tipDecText.append("请减小左手臂的幅度！")

    standard = standard_angle['elbow']['right_elbow_angle']
    angle = angle_json['elbow']['right_elbow_angle']
    if standard != -1 and angle not in interval(standard, mistake):
        if angle < standard - mistake:
            tipIncText.append("请增大右手臂的幅度！")
        if angle > standard + mistake:
            tipDecText.append("请减小右手臂的幅度！")
    ...
    if standard != -1 and angle not in interval(standard, mistake):
        if angle < standard - mistake:
            tipIncText.append("请增大左侧腰部和左大腿之间的幅度！")
        if angle > standard + mistake:
            tipDecText.append("请减小左侧腰部和左大腿之间的幅度！")

    standard = standard_angle['hip']['right_hip_angle']
    angle = angle_json['hip']['right_hip_angle']
    if standard != -1 and angle not in interval(standard, mistake):
        if angle < standard - mistake:
            tipIncText.append("请增大右侧腰部和右大腿之间的幅度！")
        if angle > standard + mistake:
            tipDecText.append("请减小右侧腰部和右大腿之间的幅度！")
    ...
    if standard != -1 and angle not in interval(standard, mistake):
        if angle < standard - mistake:
            tipIncText.append("请增大左膝盖的幅度！")
        if angle > standard + mistake:
            tipDecText.append("请减小左膝盖的幅度！")

    standard = standard_angle['knee']['right_knee_angle']
    angle = angle_json['knee']['right_knee_angle']
    if standard != -1 and angle not in interval(standard, mistake):
        if angle < standard - mistake:
            tipIncText.append("请增大右膝盖的幅度！")
        if angle > standard + mistake:
            tipDecText.append("请减小右膝盖的幅度！")

    return tipIncText, tipDecText
